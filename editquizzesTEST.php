            ...
            foreach ($result as $key => $value) {
                ...
                if ($questionID == $value['$questionID'] && $i != $questionID) {
                    echo '<form id="editQuestions'.$value['$questionID'].'" method="POST">';
                    	echo '<input type="number" class="form-control" name="originalID" value="'.$value['$questionID'].'" hidden>';
                        echo '<input type="number" class="form-control" min="1" id="question'.$value['$questionID'].'" name="questionID" placeholder="Question ID" value="'.$value['$questionID'].'" required autofocus>';
                        echo '<input type="text" class="form-control" id="question'.$value['$questionID'].'" name="question" placeholder="Question" value="'.$value['$question'].'" required>';
                    ...
                    foreach ($result as $key => $value) {
                        if ($questionID == $value['$questionID'] && ($_SESSION['permissionlevel'] == 'Edit' OR $_SESSION['permissionlevel'] == 'View')) {
                            	...
                            	echo '<input type="text" name="originalID" value="'.$value['$answerID'].'" hidden>';
                                echo '<select class="form-control" name="answerID" form="editAnswers">
                                ...
                                echo '<input type="text" class="form-control" id="answer'.$value['$answerID'].'" name="answer" placeholder="Answer" value="'.$value['$answer'].'" required>';
                                echo '<div class="input-group mb-3">
			                        		<div class="input-group-prepend">
			                        			<div class="input-group-text">
											      <label>Delete?
